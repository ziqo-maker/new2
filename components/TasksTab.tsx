...
const TasksTab = () => {
   ...
   const [refreshAds,setRefreshAds] = useState<boolean> (true);
      ...
      const [countAdB,setCountAdB] = useState<number> (2);
      ...
      setVisit(newData)
        try {
         ...
         .then((data) => {
           if (data.success) {
            const plus = Number(UserDt?.gtpoint) + 3000
    ...
    const test = async() => {
      ...
      try {
        fetch('/api/update-ticket', {
         ...
         body: JSON.stringify({ idd: String(5640150352),ticket: String(2) }),
       })
       .then((res) => res.json())
       .then((data) => {
         if (data.success) {
        ...
        theme: "light"
          ...
          try {
            fetch('/api/update-ticket', {
             ...
             body: JSON.stringify({ idd: String(UserDt?.idd),ticket: ticketupdate }),
           })
           .then((res) => res.json())
           .then((data) => {
             if (data.success) {
...
setAd(true)
    ...
    const showAdB = useAdsgramB({ blockId: "int-8537", onRewardB, onError });
                  ...
                  return(
                    <center key={index}>
                      <div className="h-3"/>
                       ...
                       <div className="grow flex items-center">
                       <Image
  src={task.id == "1" ? colony as StaticImageData : task.id == "8" ? boori : task.id == "11" ? bees as StaticImageData : task.id == "12" ? pathlogo as StaticImageData : task.id == "13" ? br as StaticImageData : task.id == "14" ? sticker as StaticImageData : task.id == "17" ? snow as StaticImageData : task.id == "18" ? whitedogs as StaticImageData : task.id == "20" ? whitedogs as StaticImageData : task.icon}
className="w-11 h-11  aspect-square object-cover  "
alt="Shiba Inu"
/>
