...
const TasksTab = () => {
           ...
           .then((data) => {
            if(data.success){
              ...
              data.all.forEach((t: any)=> {
                ...
                const gticon = t.icon == "telegram" ? telegram : twitter ;
                let model = {
                  id:t.id,
                  icon:gticon,
                  title :t.title,
    ...
    />     
              ...
              <div className=" px-1"/>
              <button onClick={() => {handleClaim(task.id,task.isDoing,gtdonetasks,pendingtasks),handlePending(task.id,pendingtasks,task.click,task.isDoing,task.url)}} className={`${task.isDoing || task.click? 'glowwhite bg-[#ffffff] animate-pulse' : 'bg-[#ffae19] glowstart' } flex-wrap z-0 rounded-full px-3 py-[3px]`}>
