    ...
    const handle = async(cost:number,id:number) => {
        ...
        if(Number(UserDt?.speedlvl)+1 == id && Number(UserDt?.gtpoint) >= Number(cost)){
            ...
            try {
             ...
             .then((data) => {
              if(data.success){
                 setUserData({idd:String(UserDt?.idd),speedlvl:String(lvl),gtpoint:String(decreasepoint),selectcharacter:String(UserDt?.selectcharacter),upgrade:String(UserDt?.upgrade),value:String(UserDt?.value),username:String(UserDt?.username),ticket:String(UserDt?.ticket),firstname:String(UserDt?.firstname)})
                 new Toast({
                               position: "top-center",
                               toastMsg: "Done.",
                               autoCloseTime: 4500,
