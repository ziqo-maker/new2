           ...
           const handlePayment = async (id:number,ton:number) => {
               ...
               if (tonConnectUI.account?.address) {
                   ...
                   try {
                     ...
                     const amount = id == 2 ? '120000000' : id == 3 ? '350000000' : id == 4 ? '470000000' : id == 5 ? '700000000' :id == 6 ? '1170000000' : '0'
    ...
    />
      ...
      alt="Shiba Inu"
       />
                 ...
                 <button onClick={() => handle(Number(tab.cost),tab.id,tab.unlock,tab.dollar,tab.ticket)} className={`${tab.unlock == true && Number(UserDt?.selectcharacter) == tab.id ? 'bg-[#ffae19]/[0.9]' : tab.unlock == true && Number(UserDt?.selectcharacter) != tab.id? 'bg-[#ffae19]/[0.5] ' : Number(UserDt?.gtpoint) >= Number(tab.cost) ? 'bg-[#ffae19]/[0.9]' : 'bg-[#ffae19]/[0.5]' } flex px-6 rounded-full border-white border-4  border-double  py-[10px] items-center justify-center text-center`}>
      ...
      className={`${tab.unlock == true && Number(UserDt?.selectcharacter) == tab.id ? 'w-6 h-6 mr-1' : 'w-0 h-0'}`}
      alt="Shiba Inu"
       />
